  ...
  const value = useMemo((): SettingsDraftContextValue | null => {
    ...
    return {
      ...
      setCompletionNotificationTimeoutSeconds: (n) =>
        patch((d) => ({
          ...d,
          completionNotificationTimeoutSeconds: Number.isFinite(n)
            ? Math.min(60, Math.max(5, Math.round(n)))
            : d.completionNotificationTimeoutSeconds,
        })),
      setCompletionNotificationPreview: (v) =>
        patch((d) => ({ ...d, completionNotificationPreview: v })),
      ...
      setCompletionNotificationDelivery: (v) =>
        patch((d) => ({ ...d, completionNotificationDelivery: v })),
